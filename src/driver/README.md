...
Native command-line arguments, selects the target architecture, and
...
| `cli.rs` | ~700 | Native CLI argument parsing: `parse_cli_args()`, query flag handling, response file expansion, and the main argument loop. |
...
| `-dumpversion` | `14` |
| `--version` | `axeon (Axon's C Compiler, Native) 14.2.0` + FSF copyright + backend mode |
...
| `-Wl,--version` (no inputs) | With `gcc_linker`: delegates to GCC; otherwise prints `GNU ld (Axon's C Compiler built-in) 2.42` for Meson linker detection |
...
Set `Axeon_TIME_PHASES=1` in the environment to print per-phase wall-clock timing
to stderr:
...
detected, it prints `GNU assembler (Axon's C Compiler built-in) 2.42` to satisfy the Linux
...
|----------|-------|---------|
| `Axeon_TIME_PHASES` | `pipeline.rs` | Print per-phase compilation timing to stderr |
| `Axeon_ASM_DEBUG` | `external_tools.rs` | Dump preprocessed assembly to `/tmp/asm_debug_<name>.s` |
| `Axeon_KEEP_ASM` | `common::temp_files`, `backend::common` | Preserve intermediate `.s` files next to output (for debugging) |
...
The `Axeon_KEEP_ASM` mechanism can override this for debugging.
