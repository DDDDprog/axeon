...
impl Driver {
    ...
    fn run_full(&self) -> Result<(), String> {
        ...
        for input_file in &self.input_files {
            if Self::is_object_or_archive(input_file) {
                ...
                let tmp = TempFile::new("axeon", Self::input_stem(input_file), "o");
                ...
                && Self::looks_like_binary_object(input_file)
            {
                ...
                return Err(format!("unknown file type: {}", input_file));
            } else {
                // When gcc_m16 feature is enabled, delegate -m16 C compilation to GCC
                #[cfg(feature = "gcc_m16")]
                if self.code16gcc {
    ...
    ///
    /// Set `Axeon_TIME_PHASES=1` in the environment to print per-phase timing to stderr.
    fn compile_to_assembly(&self, input_file: &str) -> Result<String, String> {
        let source = Self::read_source(input_file)?;

        let time_phases = std::env::var("Axeon_TIME_PHASES").is_ok();
