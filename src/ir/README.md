...
human-readable IR dump to stderr. It is activated via the `Axeon_INLINE_DUMP_IR`
...
| Variable | Effect |
|----------|--------|
| `Axeon_DISABLE_PASSES=pass1,pass2,...` | Disable specific optimization passes (e.g., `gvn,licm`). Use `all` to skip the entire optimizer. |
| `Axeon_TIME_PASSES=1` | Print per-pass timing and change counts to stderr. |
| `Axeon_INLINE_DUMP_IR=1` | Dump function IR to stderr after each inlining event. |
