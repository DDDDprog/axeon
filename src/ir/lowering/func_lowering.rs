...
impl Lowerer {
    ...
    pub(super) fn lower_function(&mut self, func: &FunctionDef) {
        ...
        let rt = func.return_type.as_ref().expect("return type required for C syntax");
        let return_is_bool = self.is_type_bool(rt);
        let base_return_type = self.type_spec_to_ir(rt);
    ...
    fn compute_function_return_type(&mut self, func: &FunctionDef) -> IrType {
        let rt = func.return_type.as_ref().expect("return type required for C syntax");
        let ret_ctype = self.type_spec_to_ctype(rt);
