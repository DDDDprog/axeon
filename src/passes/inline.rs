...
/// stops. This prevents stack frame bloat: in Axeon's codegen model, each SSA
...
/// Axeon allocates ~8 bytes per SSA value, so 200 instructions add ~1.6KB to
...
/// values in registers; Axeon's codegen spills every SSA value to the stack
...
/// helpers. Axeon's accumulator-based codegen creates one stack slot per SSA value,
...
pub fn run(module: &mut IrModule) -> usize {
    ...
    let debug_inline = std::env::var("Axeon_INLINE_DEBUG").is_ok();
    let skip_list: Vec<String> = std::env::var("Axeon_INLINE_SKIP")
        .unwrap_or_default()
        .split(',')
        .filter(|s| !s.is_empty())
    ...
    for func_idx in 0..module.functions.len() {
        ...
        for _round in 0..max_rounds {
            ...
            // Each SSA value in Axeon gets an 8-byte stack slot, so functions with
            ...
            if success {
                ...
                if std::env::var("Axeon_INLINE_VALIDATE").is_ok() {
                    validate_function_values(&module.functions[func_idx], &site.callee_name);
                }
                if std::env::var("Axeon_INLINE_DUMP_IR").is_ok() {
...
fn resolve_inline_asm_symbols(func: &mut IrFunction) {
    ...
    let debug_resolve = std::env::var("Axeon_INLINE_DEBUG").is_ok();
...
fn build_callee_map(module: &IrModule) -> HashMap<String, CalleeData> {
    ...
    let debug_callee = std::env::var("Axeon_INLINE_DEBUG").is_ok();
    for func in &module.functions {
            ...
            if let Instruction::Call { func: callee_name, info } = inst {
                if let Some(callee_data) = callee_map.get(callee_name) {
                    ...
                    if callee_name != &func.name {
                        // Skip functions listed in Axeon_INLINE_SKIP
...
fn inline_call_site(
    ...
    let debug_inline_detail = std::env::var("Axeon_INLINE_DEBUG_DETAIL").is_ok();
