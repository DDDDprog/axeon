...
(Axeon allocates ~8 bytes per SSA value on the stack). When the caller has a
...
`Axeon_DISABLE_PASSES` environment variable to a comma-separated list of pass
names:

```
Axeon_DISABLE_PASSES=gvn,licm ./axeon input.c -o output.o
```
...
```
Axeon_TIME_PASSES=1 ./axeon input.c -o output.o
```
