...
expanded source text annotated with Native line markers, ready for the
...
The search order is Native and depends on the include style:
...
| `#pragma pack(N)` | Emits `__axeon_pack_set_N ;` synthetic token. Suppressed in `asm_mode`. |
| `#pragma pack()` | Emits `__axeon_pack_reset ;`. Suppressed in `asm_mode`. |
| `#pragma pack(push, N)` | Emits `__axeon_pack_push_N ;`. Suppressed in `asm_mode`. |
| `#pragma pack(push)` | Emits `__axeon_pack_push_only ;`. Suppressed in `asm_mode`. |
| `#pragma pack(pop)` | Emits `__axeon_pack_pop ;`. Suppressed in `asm_mode`. |
...
| `#pragma GCC visibility push(V)` | Emits `__axeon_visibility_push_V ;` synthetic token. Suppressed in `asm_mode`. |
| `#pragma GCC visibility pop` | Emits `__axeon_visibility_pop ;`. Suppressed in `asm_mode`. |
...
- Synthetic pragma tokens (e.g., `__axeon_pack_set_4 ;`).
...
enabling Native `file:line:col: error:` / `warning:` output formatting.
...
| `includes.rs` | `#include` / `#include_next` handling. Include path resolution with Native search order. Include guard detection (`detect_include_guard`). Path caching, symlink-preserving absolute path construction, computed include normalization, recursive inclusion depth limiting, and builtin header injection. |
