...
impl Parser {
    /// Parse modern Axeon let statement: let x = expr; or let mut x = expr;
    pub(super) fn parse_modern_let_stmt(&mut self) -> Stmt {
        let start = self.peek_span();
        
        // Consume 'let'
        self.advance();
        
        // Check for 'mut' - handle both LetMut token and identifier "mut"
        let is_mutable = match self.peek() {
            TokenKind::LetMut => {
                self.advance();
                true
            }
            TokenKind::Identifier(n) if n == "mut" => {
                self.advance();
                true
            }
            _ => false
        ...
        let name = match self.peek() {
            ...
            _ => {
                self.diagnostics.error("expected variable name", self.peek_span());
                return Stmt::Expr(None);
            }
        };
        
        // Skip optional colon (type annotation: let x: int = ...)
        if matches!(self.peek(), TokenKind::Colon) {
            self.advance();
            // Parse type (but ignore for now - use inferred type)
        ...
        if matches!(self.peek(), TokenKind::Assign) {
            ...
            self.diagnostics.error("expected '=' after variable", self.peek_span());
        ...
        // Convert to C declaration: int x = value;
        let init = Initializer::Expr(value);
        ...
        Stmt::Declaration(Declaration::new(
            TypeSpecifier::Int,
            vec![init_decl],
            None,                    // alignment
            None,                   // alignas_type
            None,                   // alignment_sizeof_type
            AddressSpace::Default, // address_space
            None,                   // vector_size
            None,                   // ext_vector_nelem
            start,
        ))
    }
    ...
    pub(super) fn parse_stmt(&mut self) -> Stmt {
        ...
        // Handle modern Axeon: let x = expr;
        if matches!(self.peek(), TokenKind::Let) {
            return self.parse_modern_let_stmt();
        }
