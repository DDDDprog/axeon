...
impl Parser {
    pub(super) fn parse_external_decl(&mut self) -> Option<ExternalDecl> {
        ...
        // Handle modern Axeon syntax: fn name() { ... }
        if matches!(self.peek(), TokenKind::Fn) {
            return self.parse_modern_function();
        }
    ...
    /// Parse modern Axeon syntax: fn name(params) -> return_type { body }
    fn parse_modern_function(&mut self) -> Option<ExternalDecl> {
        let start = self.peek_span();
        ...
        let name = match self.peek() {
            ...
            _ => {
                self.diagnostics.error("expected function name", self.peek_span());
                return None;
            }
        };
        ...
        if matches!(self.peek(), TokenKind::LParen) {
            ...
            while !matches!(self.peek(), TokenKind::RParen | TokenKind::Eof) {
                ...
                // Skip comma at start (handles leading comma: fn(, a))
                ...
                // Parse type (or use default int)
                let type_spec = self.parse_type_specifier().unwrap_or(TypeSpecifier::Int);
                ...
                // Handle comma between params: consume and continue to next param
                if matches!(self.peek(), TokenKind::Comma) {
                    self.advance();
                }
            }
        ...
        let return_type = if matches!(self.peek(), TokenKind::ReturnArrow) {
            self.advance();
            self.parse_type_specifier()
        } else {
            Some(TypeSpecifier::Int) // Default to int
        };
        ...
        let body = if matches!(self.peek(), TokenKind::LBrace) {
            ...
            self.diagnostics.error("expected function body", self.peek_span());
            return None;
        };
