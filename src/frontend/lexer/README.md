...
The preprocessor rewrites `#pragma pack(...)` and `#pragma GCC visibility ...` directives into synthetic identifier tokens (`__axeon_pack_set_N`, `__axeon_visibility_push_hidden`, etc.). The lexer's `lex_identifier` path recognizes these by prefix and emits structured pragma token variants:
